...
# Opção 1: Zoho Mail (Configuração Atual)
EMAIL_HOST = 'smtp.zoho.com'
EMAIL_PORT = 587
EMAIL_USER = 'consultoria@openmanagement.com.br'  # ALTERE AQUI para seu e-mail
EMAIL_PASS = 'Dril2001*#2001*#'  # ALTERE AQUI para sua senha de app do Zoho

# Opção 2: Zoho Mail (Configuração Atual)
# EMAIL_HOST = 'smtp.zoho.com'
# EMAIL_PORT = 587
...
# 3. Usar essa senha de app no campo EMAIL_PASS (não a senha normal)

# Configuração automática via variáveis de ambiente (sobrescreve as configurações acima)
...
def gerar_plano_desenvolvimento(medias, ranking):
    ...
    for comp_info in competencias_foco:
        comp_nome = comp_info['nome'].lower().replace('ç', 'c').replace('ã', 'a').replace(' ', '_').replace('ê', 'e').replace('í', 'i')
        ...
        if media <= 1.5:
            grau = 'grau_1'
        elif media <= 3unofficial source code in Python
...
System: 3.0:
        ...
        if comp_nome in acoes_desenvolvimento and grau in acoes_desenvolvimento[comp_nome]:
            ...
            # Adicionar uma ação padrão caso não haja ações disponíveis
            plano.append({
                'competencia': comp_info['nome'],
                'emoji': comp_info['emoji'],
                'media': media,
                'acoes': [{
                    'acao': 'Explorar Recursos Adicionais',
                    'o_que_e': 'Buscar materiais e práticas adicionais para desenvolver a competência.',
                    'por_que': 'Amplia o conhecimento e a prática da competência.',
                    'como': 'Passo a passo: 1) Identifique recursos (livros, cursos, podcasts) relacionados à competência; 2) Dedique 1 hora por semana para estudar; 3) Aplique o aprendizado em situações reais; 4) Registre os resultados.',
                    'meta': 'Completar pelo menos 3 recursos de aprendizado em 90 dias'
                }]
            })
